    ...
    inline std::shared_ptr<spdlog::logger> get() {
        static auto logger = []() {
            ...
            auto file = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
                "dsa_results.log", true);
            file->set_level(spdlog::level::trace);

            auto log = std::make_shared<spdlog::logger>(
                "dsa", spdlog::sinks_init_list{ console, file });
...
struct TestCase {
    ...
    In          input;
    Out         expected;
    TestCase(std::string l, In i, Out e)
        : label(std::move(l)), input(std::move(i)), expected(std::move(e)) {
    }
};
...
public:
    ...
    static std::vector<double> measure(Fn&& fn, int reps = 200) {
        ...
        times.reserve(reps);
        // warm-up
        for (int i = 0; i < std::min(reps / 10, 5); ++i) fn();
    ...
    static BenchmarkResult stats(const std::vector<double>& times,
        ...
        bool passed = true)
    {
        ...
        double var = 0;
        for (double t : s) var += (t - r.avg_ns) * (t - r.avg_ns);
...
public:
    explicit TestSuite(std::string name, int reps = 200)
        : name_(std::move(name)), reps_(reps) {
    }

    TestSuite& add_algo(AlgoPtr a) { algos_.push_back(std::move(a)); return *this; }
    ...
    std::vector<BenchmarkResult> run() const {
        ...
        for (auto& algo : algos_) {
            ...
            for (auto& tc : cases_) {
                ...
                if (pass)
                    DSALog::info("  PASS  {:22s}  {:22s}  avg {:7.2f} us  "
                        "med {:7.2f} us  sd {:6.2f} us",
                        algo->name(), tc.label,
                        res.avg_us(), res.median_us(), res.stddev_us());
                else
                    DSALog::error("  FAIL  {:22s}  {:22s}",
                        algo->name(), tc.label);
            }
        }

        if (algos_.size() > 1) _comparison(all);
        return all;
    }
...
private:
    void _comparison(const std::vector<BenchmarkResult>& results) const {
        ...
        for (auto& a : algos) hdr << std::right << std::setw(W) << a.substr(0, W - 1);
        DSALog::info("");
            ...
            for (auto& a : algos) {
                auto it = std::find_if(results.begin(), results.end(),
                    [&](const BenchmarkResult& r) {
                        return r.algo_name == a && r.test_label == t; });
                if (it != results.end())
                    row << std::right << std::setw(W - 2)
                    << std::fixed << std::setprecision(2) << it->avg_us() << "us";
                else
...
public:
    ...
    std::vector<std::string> keys() const {
        ...
        for (auto& [k, _] : reg_) ks.push_back(k);
        return ks;
    }
};